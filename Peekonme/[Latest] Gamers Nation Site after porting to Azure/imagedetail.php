    ...
    <body>
        <?php
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") 
			{
                ...
                $sql = "INSERT INTO review_have (GameID,UserID,Rating,Comment,TimeStamp) VALUES ('".$imageid."','".$_SESSION['username']."','".$gameRating."','".$gameComment."','".$currentdt."')";
						...
						<li>
							...
							<?php                       
                                ...
                                if ($result = sqlsrv_query($conn, $sql)) 
                                {
                                    ...
                                    {
                                        echo '<div>';
                                        if ((isset($_SESSION['username'])) && $_SESSION['username'] == $row['UserID']) 
                                        {
                                            echo '<p class="text-right"><a href="deletereview.php?game='.$imageid.'&user='.$_SESSION['username'].'" class="btn btn-danger" role="button">Delete</a></p>';
                                        }
                                        echo 'Rating: '. $row['Rating'] .' / 5';
                                        echo '<br>';
                                        $timestamp = date_format( $row['TimeStamp'], 'Y-m-d H:i:s' );
                                        echo 'Commented on: ['. $timestamp .'] ';
                                        echo '<br>';
                            ...
                            <li>
                                <form class="form-horizontal" id="Updatecomment" role="form" method="post" action="imagedetail.php?id=<?php echo $imageid ?>">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <a href="explore.php" class="btn btn-danger btn-md col-md-5">Cancel</a>
                                    </div>
                                </form>                                        
                            </li>
                            <?php
