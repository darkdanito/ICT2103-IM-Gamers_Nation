...
<html>
    ...
    <body>
        ...
        <div class="container" style="margin-top: 4em ">
          ...
          <div class="row">
            <?php
                    ...
                    $sql = "select g.*, cast(avg(r1.rating) as decimal(10,2)) as Avg_Rating, r3.comment as Comment from review_have r1
                    full outer join game g on g.gameid = r1.gameid
                    ";
                    //DATEDIFF(MONTH,r1.timestamp,getdate()) = 3
                    //filter by rating made in the last x months
                    if ($_POST['rFilter'] != 'All'){
                    $sql.=" ".$_POST['o2Filter']." DATEDIFF(MONTH,r1.timestamp,getdate()) <= ". $_POST['rFilter'];}
                    
                    $sql.=" full outer join review_have r3 on r1.gameid = r3.gameid";
                    $sql.=" full outer join(select gameid, min(rating) as minRating, max(rating) as maxRating, min(timestamp) as minTimestamp, max(timestamp) as maxTimestamp from review_have group by gameid)r4 on r3.gameid=r4.gameid";
                    
                    
                    //search based on similar game title
                    if (isset($_GET['typeahead'])){
                      $sql .= " where g.title like '" . "%" . $_GET['typeahead']. "%" . "'";}
            ...
            default:
                echo "Unexpected case, selection not captured!";
                    ...
                    //group by gameid
                    $sql .= " group by g.gameid, g.title, g.publisher, g.Year_Released, g.Platform, g.Region, g.Price, g.ImagePath, r3.Comment";
                    echo $sql;
                    $loop = sqlsrv_query($conn, $sql)
        ...
        <strong>Comment</strong>
                        ...
                        $total_records = 0;
                            while ($record = sqlsrv_fetch_array($loop, SQLSRV_FETCH_ASSOC))
                            {
                              $total_records++;
                                    echo '<tr>';
                                    echo '<td>';
